...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2,2), trainable=True) :
    ...
    x = Convolution2D(nb_filter1, (1, 1), strides=strides, name=conv_name_base + '2a', trainble=trainable)(input_tensor)
    x = FixedBatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = Convolution2D(nb_filter2, (kernel, kernel_size), padding='same', name=conv_name_base + '2b', trainble=trainable)(x)
    x = FixedBatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = Convolution2D(nb_filter3, (1, 1), name=conv_name_base + '2c', trainble=trainable)(x)
    x = FixedBatchNormalization(axis=bn_axis, name=bn_name_base + '2c')(x)

    shortcut = Convolution2D(nb_filter3, (1, 1), strides=strides, name=conv_name_base + '1', trainble=trainable)(input_tensor)
...
def NN_base(input_tesnor=None, trainable=False) :
    ...
    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', trainble=trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', trainable=trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', trainable=trainable)

    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', trainble=trainable)
...
def rpn(base_layers, num_anchors) :
    ...
    x = Convolution2D(512, (3, 3), paddig='same', activation='relu', kernel_initializer='normal', name='rpn_conv1')(base_layers)
